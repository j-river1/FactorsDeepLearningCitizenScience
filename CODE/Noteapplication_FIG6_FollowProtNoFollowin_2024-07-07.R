...
plot_vertical <- ggplot(plot_frame, aes(x = yoloV8.x, y = rama_re.x)) +
  geom_point(alpha = 0.5,aes(color = density)) +
  scale_color_viridis() +
  # Add points with transparency       # Add 2D density contours
  labs(y = "Total number of coffee cherries manually \n counted on the branch", 
       x = "Total number of coffee cherries detected per picture \n by YOLO v8") +  # Customize labels
  facet_wrap(~errores_protocol, ncol =  1) +
  theme_bw() +
  theme(
    axis.title = element_text(size = 14),  # Tamaño del título de los ejes
    axis.text = element_text(size = 14),   # Tamaño del texto de los ejes
    strip.text = element_text(size = 14),  # Tamaño del texto en las facetas
    legend.text = element_text(size = 14), # Tamaño del texto en la leyenda
...
ggsave("results/FIG2_FolloNotFollowVertical_2024-11-20_v02.svg", plot = plot_vertical , width = 5.91, dpi = 300)
ggsave("results/FIG2_FolloNotFollowVertical_2024-11-20_v01.jpg", plot = plot_vertical , width = 5.91, dpi = 300)
