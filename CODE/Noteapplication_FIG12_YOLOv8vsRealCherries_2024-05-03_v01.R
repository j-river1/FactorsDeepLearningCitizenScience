...
# df_ <- read.csv("data/DataBase_2024-04-16_v38.csv")

df_ <- read.csv("data/DataBase_2024-05-16_v49.csv")

colnames(df_)

# df_$errorYOLOv8Resta <- df_$yoloV8.x - df_$rama_re.x
# #
# write.csv(df_, "data/DataBase_2024-05-03_v39.csv", row.names = F)
...
fit <- lm(rama_re.x ~ yoloV8.x, data = plot_frame) # fit the model
plot_frame$predicted <- predict(fit)   # Save the predicted values
plot_frame$residuals <- residuals(fit) # Save the residual values
...
plot <- ggplot(plot_frame, aes(x = yoloV8.x, y = rama_re.x)) +
  ...
  labs(y = "Number of coffee cherries manually counted in the field", 
       x = "Number of coffee cherries YOLO v8 detected per picture") +  # Customize labels
  theme_bw() +
  theme(
  ...
  geom_segment(aes(xend = yoloV8.x, yend = yoloV8.x), alpha = .2, color = "red", size =1.2) + 
  stat_poly_eq(label.y = "bottom", label.x = "right") +# Adjust scales as needed+
  labs(linetype = "Legend Title") +
  scale_linetype_manual(name = "Legend Title", values = "dashed", labels = "Explanation of dashed line")
...
ggsave("results/FIG11_YOLOv8vsReal_2024-05-03_v04.svg", plot = plot, width = 6, height = 6, dpi = 300)
