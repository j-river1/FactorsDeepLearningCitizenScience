...
tree_model <- partykit::ctree( as.numeric(errorYOLOv8Resta) ~ Variedad_ + BehindBranch_ + LocationBranch_ + Instructions_ + 
                                 XResolution + longitude + latitude + ImageWidth + ImageHeight+ AnglePicture_ + YResolution + Flash_  + Photographer + ScreenWidth + ScreenHight+ ResolutionFrontCamerainMP_ + Features_, data = df_)

partykit::varimp(tree_model)

svglite(filename='results/FIG8_ALLCtree_2024-07-05_v06.svg')
plot(tree_model, cex.lab = 1, cex.axis = 0.5, cex.main = 2, gp =gpar(fontsize = 9))
dev.off()
