...
df_ <-  read.csv("data/DataBase_2024-05-15_v48.csv")
colnames(df_)
...
df_$Variedad_ <- factor(df_$Variedad_)
df_$BehindBranch_ <- factor(df_$BehindBranch_)
df_$LocationBranch_ <- factor(df_$LocationBranch_)
df_$Instructions_ <- factor(df_$Instructions_)
df_$XResolution <- as.numeric(df_$XResolution)
df_$YResolution <- as.numeric(df_$YResolution)
df_$Flash_ <- as.numeric(df_$Flash_)
df_$Features_ <- factor(df_$Features_)
df_$AnglePicture_ <- factor(df_$AnglePicture_)
...
tree_model <- partykit::ctree( as.numeric(errorYOLOv8Resta) ~ Variedad_ + BehindBranch_ + LocationBranch_ + Instructions_ + 
                                 XResolution + AnglePicture_ + YResolution + Flash_  + Photographer + ScreenWidth + ScreenHight+ ResolutionFrontCamerainMP_ + Features_, data = df_)

partykit::varimp(tree_model)

svglite(filename='results/FIG8_ALLCtree_2024-05-15_v05.svg')
plot(tree_model, cex.lab = 1, cex.axis = 0.5, cex.main = 2, gp =gpar(fontsize = 9))
dev.off()
